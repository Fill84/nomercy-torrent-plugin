...
/// call is what puts a show in a library, and it is the only thing that does.
/// The encoder does not: <c>PluginEncoder</c> puts the media id straight into
/// <c>VideoEncodeJob.Id</c> and that job resolves it against <c>Movies.Id</c> or
/// <c>Episodes.Id</c> and nothing else. Nothing here invents anything — it asks
/// the same probe and dispatches the same job.
/// </para>
/// <para>
/// <strong>Nothing else happens on that tick.</strong> The import runs on the
/// server's own queue, so the show is not there the moment it is asked for. The
/// tick after it lands sees an ordinary grab: matched by name, covered, staged,
/// and dispatched by each episode's own id.
/// </para>
/// <para>
...
/// name an episode the server already has and offers no way to ask a provider
/// anything or to queue one of the server's own jobs, so there is nothing to
/// call. Every step is guarded and every failure is one line the owner can
/// read, because a server that renames one of these types will break it — and
/// when it does, the plugin says so and carries on rather than falling over.
/// </para>
/// </remarks>
public sealed class ShowImport(IServiceProvider services, ILogger logger) : IShowImport
{
    ...
    public void Ready()
    {
        ...
        if (missing.Length > 0)
        {
            logger.LogWarning(
                ...
                + "A torrent for one is left where it is, with the show it names written on the History page.",
                string.Join(", ", missing));
        ...
        if (Resolve(ProbeType) is null || Resolve(DispatcherType) is null)
        {
            logger.LogWarning(
                ...
                + "so a torrent for a show the owner does not have is left where it is.");
    ...
    public async Task<string?> AddAsync(string title, int? year, Library into, CancellationToken ct)
    {
        ...
        return Dispatch(dispatcher, job, id, into, matched ?? title);
    }
    ...
    private string? Dispatch(object dispatcher, Type job, int id, Library into, string matched)
    {
        ...
        try
        {
            object? libraryId = dispatch.GetParameters()[1].ParameterType
                ...
                .Invoke(null, [into.Id, CultureInfo.InvariantCulture]);

            if (libraryId is null)
            {
                logger.LogWarning("{Library} is not an id this server can read.", into.Name);
